...
int Searcher::alphabeta(int depth, int ply, int alpha, int beta, bool nmp,
  ...
  if (tthit) {
    ...
    if (ttdepth >= depth) {
      ...
      int margin = std::max(20 * isttPV + 20 * isPV, 70 * depth - 70 * ttdepth - 70 * improving);
  ...
  int margin = std::max(20 * isttPV + 20 * isPV, 70 * depth - 70 * improving);
  if (ply > 0 && !tthit) {
  ...
  if (((update || (realnodetype == EXPECTED_PV_NODE)) && !(*stopsearch))) {
    ttentry.update(Bitboards.zobristhash, Bitboards.gamelength, depth, ply,
                   isttPV, bestscore, realnodetype, savedmove);
  }
  return bestscore;
}
